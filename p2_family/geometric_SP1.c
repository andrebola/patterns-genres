...
static char EFFECT[] = {
  0, 1, 2, 3, 4,  6, 13, 25, 38,  50, 38, 25, 13,  6, 4, 3, 2, 1, 0,
  1, 2, 3, 4, 6, 12, 25, 50, 75, 100, 75, 50, 25, 12, 6, 4, 3, 2, 1,
  0, 1, 2, 3, 4,  6, 13, 25, 38,  50, 38, 25, 13,  6, 4, 3, 2, 1, 0
};

/*  4,  5, 10, 13, 20, 25, 30, 38, 45,  50, 45, 38, 30, 25, 20, 13, 10,  5,  4,
  8, 13, 20, 25, 40, 50, 65, 75, 90, 100, 90, 75, 65, 50, 40, 25, 20, 13,  8,
  4,  5, 10, 13, 20, 25, 30, 38, 45,  50, 45, 38, 30, 25, 20, 13, 10,  5,  4*/
...
/**
 * Search a song collection with c_geometric_sp1_scan().
 *
 ...
 */
void alg_sp1(const songcollection *sc, const song *pattern, int alg,
        const searchparameters *parameters, matchset *ms) {
    int i;
    for (i=0; i<sc->size; ++i) {
        scan_song_sp1(&sc->songs[i], pattern, ms);
    }
}
 ...
 */
int scan_song_sp1(const song *s, const song *p, matchset *ms) {
    int   i;
    ...
    for (i=0; i<song_end; ++i) {
        ...
        for (j=0; j<=MIN2(MAX_INITIAL_SKIP, p->size - 2); ++j) {
            ...
            for (; hash_x <= hash_x_end; ++hash_x, pos += NOTE_PITCHES) {
                ...
                int skips = 0;
#endif
#ifdef FOLLOW_INSTRUMENT
                int last_instrument = snotes[i].instrument;
#endif
...
#endif
#ifdef FOLLOW_INSTRUMENT
                        if (last_instrument != snotes[sp].instrument) {
                            sim = 0.0F;
                            last_instrument = snotes[sp].instrument;
                        }
#endif
                        similarity += sim;
