                        ...
                        for key in tonic_pattern.keys():
                            ...
                            if key not in ["jsDsRhythm", "dsChroma"]:
                                ...
                                if len(normalized_patterns):
                                    ...
                                    genres_patterns_labels[key].append(genre)
                                    genres_patterns[key].append(list(normalized_patterns))
        except ValueError:
            pass
...
def classify(genres_patterns, genres_patterns_labels):
    ...
    for key in genres_patterns.keys():
        ...
        conv_X_train = mlb.fit_transform(genres_patterns[key])

        np.delete(conv_X_train, np.where(conv_X_train.sum(1)<5), 1)
...
def main():
    # Load json file with annotations of genre for each midi file
    genres_map = json.load(open("midi_genre_map.json"))
    # Load json with Beat Per Quaerter Note for each midi file
    tpqn_map = json.load(open("tpqn.json"))
    # Location of patterns extracted from Midi files
    p2path = "patterns_p2/chroma/"
    p2path2 = "patterns_p2/rhythm/"

    genres_patterns, genres_patterns_labels = load_data(p2path, None, genres_map, tpqn_map)
    results = classify(genres_patterns, genres_patterns_labels)
    json.dump(results, open("classification_p2_res_tonic_logi.json", "w"))
    genres_patterns, genres_patterns_labels = load_data(p2path, p2path2, genres_map, tpqn_map)
    results = classify(genres_patterns, genres_patterns_labels)
    json.dump(results, open("classification_p2_res_rhythm_and_tonic_logi.json", "w"))

    # Count the different patterns
    """results = count_patterns(genres_patterns, genres_patterns_labels)
    json.dump(results, open("count_patterns_p2_only_tonic.json", "w"))
    genres_patterns, genres_patterns_labels = load_data(p2path, p2path2, genres_map, tpqn_map)
    results = count_patterns(genres_patterns, genres_patterns_labels)
    json.dump(results, open("count_patterns_p2_rhythm_and_tonic.json", "w"))
    """
